...
- **NEVER run `git commit` (or amend, or any command that creates a commit) without my explicit approval each time.** This applies at all times, in every workflow — including multi-step skills and automated pipelines. Pipeline/skill authorization does NOT imply commit authorization.
